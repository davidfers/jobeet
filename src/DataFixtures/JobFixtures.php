...
class JobFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager) : void
    {
        ...
        for ($i = 100; $i <= 200; $i++) {
            $categoryArray = array('category-programming', 'category-design', 'category-manager', 'category-administrator');
            $categorySelected= array_rand(array_flip($categoryArray), 1);
            $job = new Job();
            $job->setCategory($manager->merge($this->getReference($categorySelected)));
